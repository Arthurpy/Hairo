<template>
    <form class="mx-auto flex w-full max-w-lg flex-col rounded-xl border border-border bg-backgroundSecondary p-20 bg-white pb-[2rem] pt-[2rem]" @submit.prevent="signup">
        <div class="flex w-full flex-col gap-2 bg-grey blur-[1px] p-5 rounded-[10px]">
            <p class="text-black">Sign up with</p>
            <div class="flex w-full flex-col gap-2 bg-slate-500 p-4">
			  <button type="button" class="btn gap-2 ">
			    ...
			    <span class="text-white">S'inscrire avec google</span>
			  </button>
			  <button type="button" class="btn gap-2 bg-gray-5">
			    ...
			    <span class="text-white">S'inscrire avec twitter</span>
			  </button>
			  <button type="button" class="btn gap-2 bg-gray-5">
        ...
        <div class="divider my-6 text-xs text-content2">ou S'inscrire avec</div>

        <div class="form-group">
            <div class="form-field">
                <label class="form-label text-black">Addresse email</label>
            ...
            <div class="form-field">
                <label class="form-label text-black">Mot de passse</label>
                <input v-model="password" placeholder="Mot de passse" type="password" class="input max-w-full" />
            </div>
            <div class="form-field">
                <label class="form-label text-black">Confirme le mot de passse</label>
                <input v-model="confirmPassword" placeholder="Confirme le mot de passse" type="password" class="input max-w-full" />
                <p v-if="passwordError" class="text-red-500">{{ passwordErrorMessage }}</p>
            </div>
            <div class="form-field pt-5">
                <button type="submit" class="btn btn-primary w-full" @click="register">S'inscrire</button>
            </div>
            <p v-if="signupError" class="text-red-500">{{ signupErrorMessage }}</p>
            <div class="form-field">
                <button @click="goToLogin">
                    <a class="link link-underline-hover link-primary text-sm">Déja un compte? Se connecter</a>
                </button>
            </div>
        </div>
    </form>
    ...
    methods: {
        validateSignup() {
            ...
            if (this.password !== this.confirmPassword || !this.password) {
                ...
                this.passwordErrorMessage = 'Passwords do not match or are empty.';
                return false;
            } else {
